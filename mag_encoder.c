...
//            } else if ((data >> 1) & 0x1) {
//                UARTprintf("Command Invalid \n");
//            } else if (data & 0x1) {
//                UARTprintf("Framing Error \n");
//            }
//            break;
//        }
//    }
...
uint32_t readRegister(uint32_t addr) {
    /* Sends read command for given register, returns data from previous command*/
    uint32_t data;
    data = CMD_READ | addr;
    data |= calcEvenParity(data) << 15;
    SSIDataPut(SSI0_BASE, data);
    ...
    *angle = (value * 360) / 16384; // Angle value in degrees (0..360)
    ...
    for (i = 1; i < 4; i++) {
        ...
        /* error flag set */
            ...
            // Get error register contents
            recBuffer[4] = sendNOP();
            if ((recBuffer[4] >> 2) & 0x1){
                UARTprintf("Parity Error \n");
            } else if ((recBuffer[4] >> 1) & 0x1) {
                UARTprintf("Command Invalid \n");
            } else if (recBuffer[4] & 0x1) {
                UARTprintf("Framing Error \n");
            }
            break;
        }
...
void readAverageData(uint32_t* angle, uint32_t* mag, uint32_t* agc){
    ...
    *mag = *mag/AVG_CYCLES;
    *agc = *agc/AVG_CYCLES;
//    alarmHi = alarmHi/AVG_CYCLES;
//    alarmLo = alarmLo/AVG_CYCLES;
    if (alarmHi){
